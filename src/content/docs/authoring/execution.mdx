...
| `SERVICE_URL_PATH` | Service-type jobs only | Absolute path of the service URL file. Manual services write their URL here; for `auto_url` services Fileglancer writes it after the port is ready. Equivalent to `$FG_WORK_DIR/service_url` (see [Services](/authoring/services/)) |

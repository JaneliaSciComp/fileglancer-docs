...
| `file` | Text input + file browser | `--flag '/path/to/file'` | `'/path/to/file'` | Must be an absolute path that exists and is readable on the server (unless `exists: false`) |
| `directory` | Text input + directory browser | `--flag '/path/to/dir'` | `'/path/to/dir'` | Must be an absolute path that exists and is readable on the server (unless `exists: false`) |
...
- Paths are validated server-side before job submission (must exist and be accessible, unless `exists: false`).

...
| `create_if_missing` | boolean | no | Only valid on `directory` parameters. If `true`, Fileglancer creates the directory (as the user, within an allowed file share) before the existence check, so a home default like `~/.fileglancer/logs` works on first launch. Default: `false` |
...
- `directory` parameters can set `create_if_missing: true`. Fileglancer then creates the resolved directory (as the user, within an allowed file share) just before the existence check, so a default like `~/.fileglancer/logs` works out of the box and overrides that point at a new directory are created too. Overrides pointing at an existing directory are a no-op. This never creates anything outside a file share. Useful for a default output or logs directory.
